...
def read_order_data(city_name):
    ...
    # print(name)
    df = pd.read_csv(name)
    df = df.groupby(['date_','city_name','item_id','item_name']).agg({'sales_quantity':'sum','sales_value':'sum','sales_value_mrp':'sum'}).reset_index()
...
def get_error_data(order_data):
    ...
    error_data = error_data[['city_name', 'item_id', 'min', 'max', 'error_reason']]
    return error_data
...
def handle_error_transformation(city_error_df, city_name, item_id, order_data):
    df = order_data[order_data['item_id'] == item_id]
    min_val = df['sales_quantity'].min()
    max_val = df['sales_quantity'].max()
    new_row = [city_name,item_id,round(min_val,2),round(max_val,2),'Error occurred during transformation']
    temp_df = pd.DataFrame([new_row],columns=['city_name', 'item_id', 'min', 'max', 'error_reason'])
    city_error_df = pd.concat([city_error_df, temp_df], ignore_index=True)

    return city_error_df


def handle_error_modeling(city_error_df, city_name, item_id, order_data):
    df = order_data[order_data['item_id'] == item_id]
    min_val = df['sales_quantity'].min()
    max_val = df['sales_quantity'].max()
    new_row = [city_name, item_id, round(min_val, 2), round(max_val, 2), 'Error occurred during modeling']
    temp_df = pd.DataFrame([new_row], columns=['city_name', 'item_id', 'min', 'max', 'error_reason'])
    city_error_df = pd.concat([city_error_df, temp_df], ignore_index=True)
...
def process_city(city_name):
    ...
    print(city_name, 'Total Combinations:', (len(set(order_data['item_id']))))
    order_data['Sales'] = order_data['sales_quantity']
    # print(order_data.head())
    # error_data = get_error_data(order_data)
    # print(error_data.head())
    error_data = pd.DataFrame(columns=['city_name', 'item_id', 'min', 'max', 'error_reason'])
    order_data = preprocess_order_data(order_data)
    # print(order_data.head())

    city_final_df = pd.DataFrame(columns=['city_name', 'item_id', 'min', 'max'])
    city_error_df = pd.DataFrame(columns=['city_name', 'item_id', 'min', 'max', 'error_reason'])
    outlier_df = pd.DataFrame(columns=['date_', 'city_name', 'item_id', 'item_name', 'sales_quantity', 'Sales', 'outlier_city_item'])
    ...
    for item_id in get_unique_store_item_pairs(order_data):
        ...
        try:
            ...
            error_data = handle_error_transformation(city_error_df, city_name, item_id, order_data)
            continue
        try:
            ...
            # print(transformed_sales)
            transformed_sales['outlier_city_item'] = np.where(transformed_sales['cluster'] == no_outlier_cluster, 0, 1)
            transformed_sales = transformed_sales[['date_', 'city_name', 'item_id', 'item_name', 'sales_quantity', 'Sales', 'outlier_city_item']]
            # print(transformed_sales)
            outlier_df = pd.concat([outlier_df, transformed_sales], ignore_index=True)
            # print('Failed-6')
            new_row = [city_name,item_id,round(min_val, 2),round(max_val, 2)]
            temp_df = pd.DataFrame([new_row], columns=['city_name', 'item_id', 'min', 'max'])
            # print(temp_df)
            city_final_df = pd.concat([city_final_df, temp_df], ignore_index=True)
            city_final_df.to_csv('city_final_df_city_item.csv', index=False)
            # print('Failed-8')
        except Exception:
            error_data = handle_error_modeling(error_data,city_name, item_id, order_data)
...
def main():
    ...
    final_df = pd.DataFrame(columns=['city_name', 'item_id', 'min', 'max'])
    error_df = pd.DataFrame(columns=['city_name', 'item_id', 'min', 'max', 'error_reason'])
    final_outlier_df = pd.DataFrame(columns=['date_', 'city_name', 'item_id', 'item_name', 'sales_quantity', 'Sales', 'outlier_city_item'])
    ...
    final_df.to_csv('output_city_item/final_df_b.csv', index=False)
    error_df.to_csv('output_city_item/error_df_b.csv', index=False)
    final_outlier_df.to_csv('outlier_output_city_item/final_df.csv', index=False)
