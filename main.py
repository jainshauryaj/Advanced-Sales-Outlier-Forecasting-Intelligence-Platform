...
def get_error_data(order_data):
    ...
    error_data = error_data[['city_name', 'facility_id', 'item_id', 'min', 'max', 'error_reason']]
    return error_data
...
def get_unique_store_item_pairs(order_data):
    return order_data.groupby(['facility_id', 'item_id']).groups.keys()


def apply_boxcox(order_data, store_id, item_id):
    # print(order_data)
    item_data = order_data[(order_data['facility_id'] == store_id) & (order_data['item_id'] == item_id)]
    # print('step-1')
...
def handle_error_transformation(city_error_df, city_name, store_id, item_id, order_data):
    df = order_data[(order_data['facility_id'] == store_id) & (order_data['item_id'] == item_id)]
    min_val = df['sales_quantity'].min()
    max_val = df['sales_quantity'].max()
    new_row = [city_name,store_id,item_id,round(min_val,2),round(max_val,2),'Error occurred during transformation']
    temp_df = pd.DataFrame([new_row],columns=['city_name', 'facility_id', 'item_id', 'min', 'max', 'error_reason'])
    city_error_df = pd.concat([city_error_df, temp_df], ignore_index=True)
    # city_error_df = city_error_df.append({'facility_id': store_id, 'item_id': item_id, 'min': round(min_val, 2),
    #                                       'max': round(max_val, 2),
    #                                       'error_reason': 'Error occurred during transformation'}, ignore_index=True)
    return city_error_df


def handle_error_modeling(city_error_df, city_name, store_id, item_id, order_data):
    df = order_data[(order_data['facility_id'] == store_id) & (order_data['item_id'] == item_id)]
    min_val = df['sales_quantity'].min()
    max_val = df['sales_quantity'].max()
    new_row = [city_name, store_id, item_id, round(min_val, 2), round(max_val, 2), 'Error occurred during modeling']
    temp_df = pd.DataFrame([new_row], columns=['city_name', 'facility_id', 'item_id', 'min', 'max', 'error_reason'])
    city_error_df = pd.concat([city_error_df, temp_df], ignore_index=True)
    # city_error_df = city_error_df.append({'facility_id': store_id, 'item_id': item_id, 'min': round(min_val, 2),
    #                                       'max': round(max_val, 2), 'error_reason': 'Error occurred during modeling'},
    #                                      ignore_index=True)
    return city_error_df
    ...
    column_dtypes = [
        {
            "name": "city_name",
            "type": "varchar",
            "description": "city of dark store",
        },
        {
            "name": "facility_id",
            "type": "integer",
...
def process_city(city_name):
    count = 0
    order_data = read_order_data(city_name)
    print(city_name, 'Total Combinations:', (len(set(order_data['facility_id'])) * len(set(order_data['item_id']))))
    ...
    city_final_df = pd.DataFrame(columns=['city_name', 'facility_id', 'item_id', 'min', 'max'])
    city_error_df = pd.DataFrame(columns=['city_name', 'facility_id', 'item_id', 'min', 'max', 'error_reason'])
    outlier_df = pd.DataFrame(columns=['date_', 'city_name', 'facility_id', 'item_id', 'item_name', 'sales_quantity', 'Sales', 'outlier_city_item'])

    for store_id, item_id in get_unique_store_item_pairs(order_data):
        # print(store_id, item_id)
        try:
            transformed_sales = apply_boxcox(order_data, store_id, item_id)
            # print('Failed-1')
            # imputed_sales = impute_outliers(transformed_sales)
        except Exception:
            error_data = handle_error_transformation(city_error_df, city_name, store_id, item_id, order_data)
            continue
        try:
            ...
            #
            transformed_sales['outlier_store_item'] = np.where(transformed_sales['cluster'] == no_outlier_cluster, 0, 1)
            transformed_sales = transformed_sales[['date_', 'city_name', 'facility_id', 'item_id', 'item_name', 'sales_quantity', 'Sales', 'outlier_store_item']]
            # print(transformed_sales)
            outlier_df = pd.concat([outlier_df, transformed_sales], ignore_index=True)
            # print('Failed-6')
            new_row = [city_name, store_id, item_id, round(min_val, 2), round(max_val, 2)]
            temp_df = pd.DataFrame([new_row], columns=['city_name', 'facility_id', 'item_id', 'min', 'max'])
            # print(temp_df)
            city_final_df = pd.concat([city_final_df, temp_df], ignore_index=True)
            # city_final_df = city_final_df.append(
            #     {'facility_id': int(store_id), 'item_id': int(item_id), 'min': round(min_val, 2),
            #      'max': round(max_val, 2)}, ignore_index=True)
            # print('Failed-7')
            city_final_df.to_csv('city_final_df.csv', index=False)
            # print('Failed-8')
        except Exception:
            error_data = handle_error_modeling(error_data, city_name, store_id, item_id, order_data)

        count += 1
        print(city_name, ' Processed:', count)
...
def process_city_parallel(city_name):
    # print(city_name)
    city_final_df, city_error_df, outlier_df = process_city(city_name)
...
def main():
    ...
    final_df = pd.DataFrame(columns=['city_name', 'facility_id', 'item_id', 'min', 'max'])
    error_df = pd.DataFrame(columns=['city_name', 'facility_id', 'item_id', 'min', 'max', 'error_reason'])
    final_outlier_df = pd.DataFrame(columns=['date_','city_name', 'facility_id', 'item_id', 'item_name', 'sales_quantity', 'Sales', 'outlier_store_item'])
    ...
    for city_final_df, city_error_df, outlier_df in results:
        final_df = pd.concat([final_df, city_final_df], ignore_index=True)
        error_df = pd.concat([error_df, city_error_df], ignore_index=True)
        final_outlier_df = pd.concat([final_outlier_df, outlier_df], ignore_index=True)

    final_df['city_name'] = final_df['city_name'].astype('str')
    ...
    error_df['city_name'] = error_df['city_name'].astype('str')
    ...
    error_df.to_csv('output/error_df.csv', index=False)
    final_outlier_df.to_csv('outlier_output_store_item/final_df.csv', index=False)
